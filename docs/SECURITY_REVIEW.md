...
| `~/.parleq/harvested-negatives.enc` | **Correction-harvested voiceprint negatives** (**Concord / release builds only**): bounded FIFO rings (≤ 8 per term-confusable pair) of pooled float embeddings, harvested when the user corrects a dictionary-term over-fire on an **enrolled** term (per-correction undo, or an E-edit changing the term back to a common word). Each embedding derives from the ~0.5 s audio span of the **single corrected word** in one dictation utterance — **embeddings only, never audio, never transcript text** (the confusable label keys are single words the user typed/heard, stored only inside the encrypted blob). | **Biometric-derived data, encrypted at rest.** AES-256-GCM under the **same** device-only Keychain key + `0600` atomic-write as `voiceprints.enc`. Active only for terms the user explicitly enrolled under the biometric consent (same data class as `voiceprints.enc`); kill-switch `voiceprintHarvestEnabled` (default on, user/MDM-manageable, **fail-closed** on a malformed managed value); the Settings toggle-off flow offers to clear all harvested refinements (each voiceprint returns to its enrollment state). Never network-transmitted. Deleted with the term's voiceprint / "Delete all voiceprints". See §5.4. |
...
**Always-on encoder-feature capture (Concord build).** So the gate can pool a word's embedding, the Concord build asks FluidAudio to export the Parakeet encoder's acoustic features on every transcribe call (`ASRDiagnostics.encoderFeatures`). This sequence is **transient (~0.8 MB), in process memory only, freed at end of utterance, and excluded from `Codable`** — it is never logged, never serialized to the flywheel record, and never written to disk (§5 "Explicitly NOT written to disk"). *(One scoped extension: with a voiceprint enrolled **and** harvest enabled, the review window retains the sequence until the overlay reaches a terminal state — see "Correction-time negative harvest" below. Still memory-only, still non-`Codable`.)* The gate itself is **self-gating**: with nothing enrolled it is a no-op with byte-identical baseline output.
...
**Correction-time negative harvest.** When the user corrects a dictionary-term over-fire on an **enrolled** term — the overlay's per-correction undo (⌥digit), or an in-place E-edit that changes the emitted term back to a common word — Parleq pools that corrected word's **audio-span embedding** from the utterance's in-memory encoder features and attaches it as a **negative prototype** to the term's voiceprint, converting the weak one-class template into the validated contrastive kind through real usage (no re-enrollment). The **source delta vs. enrollment**: the embedding derives from a ~0.5 s span of a **dictation** utterance (the single corrected word only, never the sentence) rather than an enrollment carrier — disclosed in the enrollment consent + Settings copy ("refines that term's voiceprint using the corrected word's sound — stored as an encrypted embedding, never audio; deleted with the voiceprint"). The raw harvested embeddings persist in bounded FIFO rings (≤ 8 per term-confusable pair) in `~/.parleq/harvested-negatives.enc` (same AES-256-GCM + device-only Keychain key + `0600` atomic write as `voiceprints.enc`; §5 table) so centroids can be recomputed and a mistaken harvest **healed** — undoing a gate revert removes the newest harvested sample via the same one-keystroke gesture. Safeguards: active only for terms enrolled under the biometric consent; kill-switch `voiceprintHarvestEnabled` (default on, user-visible in Settings, MDM-manageable, **fail-closed** on a malformed managed value) whose toggle-off flow offers to clear all harvested refinements; a phonetic-confusability gate (DoubleMetaphone agreement, or a user-declared alias / enrollment-declared confusable) so a semantic correction never poisons a template; a self-label guard (a case-only edit can never attach the term's own audio); and a conservative ordinal consistency guard that skips any ambiguous span location. To make correction-time pooling possible, the review window **retains the utterance's `ASRDiagnostics` (encoder features included) in process memory until the overlay reaches a terminal state** (accept / cancel / next dictation / refine turn / app quit) — only when a voiceprint is enrolled and harvest is enabled; users without voiceprints never pay the retention, and nothing about it touches disk or the network. Logging is count-only (`[voiceprint] harvest: negative attached for '<term>' (ring k/8)`) — the term name is a user-authored dictionary identifier (existing `[voiceprint]` practice); the confusable label is dictation-derived text and is **never logged**. Because the embedding source (a dictation-word span) differs from enrollment carriers, users who enrolled a voiceprint under a **prior** build — before this feature existed — receive a **one-time upgrade notice** on first launch that explains corrections now refine their voiceprints and points to the off-switch; new enrollees see the same disclosure on the enrollment consent card instead.
...
**Deletion.** Per-term **Remove voiceprint** (also removed when the dictionary term is deleted) and a global **Delete all voiceprints** clear the in-memory store and rewrite/remove the encrypted file — including the term's harvested-negative rings (`harvested-negatives.enc` is rewritten, or removed when nothing remains). Everything is also wiped from memory on quit.
