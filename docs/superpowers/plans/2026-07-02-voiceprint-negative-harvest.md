...
- [ ] **Step 3: Implement** on `VoiceprintCoordinator`:
  ...
  public func harvestNegative(termID: String, label: String, embedding: [Float],
      ...
      var ring = harvested.rings[termID]?[key]
          ?? HarvestRing(enrollmentPrototype: template.negatives[key],  // snapshot BEFORE first harvest
...
- [ ] **Step 3: Implement `harvestEmbedding`:**
  ```swift
  private func harvestEmbedding(word: String, rawOrdinal: Int) -> (embedding: [Float], matchCount: Int)? {
...
- [ ] **Step 3: Implement `harvestNegativeFromUndo`:**
  ...
  private func harvestNegativeFromUndo(span: CorrectionSpan, preText: String,
      ...
      guard let probe = harvestEmbedding(word: span.original, rawOrdinal: 0) else { return }
      ...
      guard let ordinal = HarvestSpanLocator.rawOrdinalForUndo(
          ...
          allSpans: allSpans, spanNumber: span.number, groupMatchCount: probe.matchCount)
...
- [ ] **Step 1: Hook after the `edited != editPreEditText` branch** (inside it, after `recordInPlaceEdit`):
  ```swift
  #if Concord
  harvestNegativesFromEdit(before: editPreEditText, after: edited,
                           spans: overlay.model.correctionSpans)
  #endif
  ```
- [ ] **Step 2: Implement:**
  ```swift
  ...
  private func harvestNegativesFromEdit(before: String, after: String, spans: [CorrectionSpan]) {
      ...
      for r in replacements {
          ...
          guard let probe = harvestEmbedding(word: r.before, rawOrdinal: 0) else { continue }
          guard let ordinal = HarvestSpanLocator.rawOrdinalForEdit(
              ...
              substitutionSpanTotal: subs.count, groupMatchCount: probe.matchCount)
          else { continue }
