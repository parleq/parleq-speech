...
- **The small-model question (Ph16/18):** a custom model to *beat the cleanup LLM* — **no** (Ph16, accuracy headroom ≈0). A small *posterior-conditioned ASR-correction* model to **approximate the LLM's dictionary-aware correction LOCALLY and skip an LLM pass per dictation — YES, motivated** (Ph18): the hand-built rule stack gets 72–75% term recovery but can't jointly hit the LLM's 89%-recall/0-over-fire corner (conf-gate over-fires 9; context-gate 2). A learned combiner of confidence+posterior+proximity+context targets that gap. Gate the build on tuning the rules to their own frontier first.
