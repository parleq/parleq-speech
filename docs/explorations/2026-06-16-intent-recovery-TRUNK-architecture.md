...
- **The small-model question (Ph16/18/19):** a custom model to *beat the cleanup LLM* — **no** (Ph16, accuracy headroom ≈0). A small *posterior-conditioned ASR-correction* model to **approximate the LLM's dictionary-aware correction LOCALLY and skip an LLM pass per dictation — motivated, now quantified** (Ph18/19). The rule stack tuned to its frontier (Ph19, best config: context-gate every recovery, near 0.7, floor 0.90) tops out at **~70% term recovery / ≥1 over-fire / 4.8% WER** and **structurally cannot reach the LLM's 89% / 0 / 2.1% corner** — no threshold combo hits 0 over-fire. So the **decision is a product-bar question:** if ~70% local recovery (no LLM, instant, raw-beating WER) is good enough as the fast default → **ship the tuned rules, no model**; if the bar is ≥~85% at ~0 over-fire → train a learned corrector (target: close a ~20pt recall gap), noting the **candidate-gen limit** (Ph11: a constrained corrector can't recover terms ASR mangles beyond grapheme+phonetic reach — only open-vocab gen / the LLM can) caps it below the LLM. **[POC tested, Ph20]** a flan-t5-small dict-conditioned corrector trains easily and is *excellent in-distribution* (95.9% recovery / 1.1% WER on synthetic) but **fails to transfer to real audio (42% on the real clips, ~raw)** — it memorized synthetic TTS confusion patterns, not real ASR ones. **The model is the easy part; the wall is DATA.** Verdict: don't build it on synthetic — it's gated on real `(raw→corrected)` pairs from the production data flywheel (`CorrectionJournal`/`LearnedStore` + LLM distillation), the *same* gate as the usage-prior (Ph10). Architecture correction: rules handle the deterministic classes (the spike model was *worse* than the join on class-1); a learned model is only for the hard class-3 residual, on real data. **[BREACHED, Ph21]** realism-enriched data takes real-clip recovery **42%→75% on the 24-clip held-out set (= the LLM's recovery on that same set; the LLM is 89% on the full 74), on-device.** The scalable engine is **error-injection**: mine the user's *real* ASR confusion (`sneak←Snyk`) from a handful of their clips, inject it into unlimited clean text → realistic training data at scale (inject-only: 58% recovery / **0 over-fire** / 5.0% WER, the clean MVP; +multi-voice → 75% but 17% over-fire). So the **data gate is LOWER than Ph20 implied** — a sparsely-populated correction journal can bootstrap a *personalized* on-device corrector. Remaining work is **precision, not recall** (over-fire 17% vs rules/LLM 0). The answer is the **hybrid**: deterministic rules (precision, class-1) + injection-trained learned model (recall, class-3) + a precision post-filter. Honest bound: single speaker, tiny test (12/12), same-speaker confusion recurrence — validates the *per-user personalized* recipe, not cross-speaker generalization. **[build path, Ph22]** `...phase22-personalization-and-data.md`. **Personalization = supplement, not retrain:** a **2.77 MB LoRA adapter** (0.89% of params), base frozen, trains in minutes on-device, refreshable from the correction journal — and it's the *precision* winner (8% over-fire vs full-finetune's 25%). On a *small* model, **free conditioning is insufficient** (flan-t5-small can't ICL) → the personal tier must be a trained LoRA refresh, not prompt-conditioning. **Data recipe (license-clean):** general base from **HyPoradise v0 + Robust-HyPoradise** (MIT/Apache, 334K+ GER pairs) + **LibriSpeech→FluidAudio** (CC-BY, our engine's real errors) + **People's Speech** slice (CC-BY); the dev-jargon-homophone niche has **no public dataset** → must be synthesized (confusion-injection, Ph21). Full arc: public GER data → strong base → injection for jargon → 2.77 MB per-user LoRA.
