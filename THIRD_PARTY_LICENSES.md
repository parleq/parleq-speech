...
|---|---|---|
| Direct SwiftPM dependencies (parleq-app) | 1 | Apache-2.0 |
| Direct SwiftPM dependencies (fluidaudio-sidecar) | 2 | Apache-2.0 |
| Transitive SwiftPM dependencies (shared) | 25 | Apache-2.0 |
...
Total unique third-party packages bundled in the .app (across both
SwiftPM trees): **28**.
...
|---|---|---|---|---|
| FluidAudio | 0.14.3 | Apache-2.0 | https://github.com/FluidInference/FluidAudio | Parakeet TDT v3 ASR + CTC keyword spotting on the Apple Neural Engine |
| Hummingbird | 2.22.0 | Apache-2.0 | https://github.com/hummingbird-project/hummingbird | HTTP server inside the bundled sidecar |
...
The bundled FluidAudio sidecar downloads CoreML model archives at
first launch. These are NOT shipped inside the .app — they live at
...
in both `parleq-app/` and `third_party/fluidaudio-sidecar/`, and
cross-referencing each `Package.resolved` against the LICENSE file in
that package's `.build/checkouts/<name>/` directory. The current
audit captures the dependency state at commit `5b7cc63` (2026-05-07).
