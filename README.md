...
- **Pluggable LLM cleanup across six providers.** On-device (Gemma 4 E4B via Apple MLX — runs in-process, no network boundary on the cleanup path, ~4 GB one-time download, ~6 GB memory while active, 12 GB+ RAM required) is the local-only option; Google Gemini (direct AI Studio API) and OpenAI (direct API) are the quick personal cloud paths; Google Vertex AI, AWS Bedrock (Claude Haiku 4.5 / GPT-OSS 120B), and Azure OpenAI are the enterprise paths. Each cloud provider supports both pasted API keys and your existing CLI session (gcloud, AWS SSO, az login). Or skip cleanup entirely and paste raw transcripts.
...
| LLMProvider | Provider-agnostic streaming cleanup interface — implementations for on-device MLX (LocalLLMProvider: Gemma 4 E4B, in-process, no network boundary, ~4 GB download, 12 GB+ RAM), Gemini, OpenAI, Vertex AI, Bedrock, Azure OpenAI | `parleq-app/Sources/ParleqApp/{LLMProvider,LLMClient,LocalLLMProvider,VertexProvider,BedrockProvider,BedrockBearerProvider,AzureOpenAIProvider}.swift` |
