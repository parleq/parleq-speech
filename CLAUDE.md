...
1. **Audio is memory-only end-to-end.** `AudioRecorder.stop()` returns a `Data`. There is no URL-based path. **Do not** add `/tmp/parleq-*.wav` writes or any other audio-on-disk persistence — this is a load-bearing compliance promise. *(One documented carve-out: the opt-in, hidden **contribution-capture mode** writes audio to `~/.parleq/flywheel/` — armed only by hand-editing an exact acknowledgment phrase, off by default, never network-transmitted, absent from all user-facing surfaces. The promise holds literally for every user who has not armed it. See `ContributionRecorder.swift` + `docs/SECURITY_REVIEW.md` §5.3.)*
