...
- **MLX Swift** (`mlx-swift` + `mlx-swift-lm` + `swift-transformers` + `swift-huggingface`) for the on-device cleanup option — in-process LLM inference on the Mac's GPU/ANE, the same "runs in-process, no network boundary" model as FluidAudio. The prebuilt Metal shader library (`mlx.metallib`) is fetched at build time by `parleq-app/scripts/fetch-metallib.sh` (SHA-256-verified; must match the pinned `mlx-swift` version — see build commands below and the metallib gotcha in Active gotchas).
...
The `make build` / `make install` / `make build-debug` paths all invoke `parleq-app/scripts/make-app.sh`, which runs `parleq-app/scripts/fetch-metallib.sh` automatically to stage the prebuilt MLX Metal shader library (`mlx.metallib`) into the bundle before codesign. This step requires internet access on a fresh checkout (it downloads from `github.com/ml-explore/mlx-swift/releases`); the downloaded library is cached in `parleq-app/.build-cache/` for subsequent builds. `swift build` alone (without make-app.sh) does NOT stage the metallib — the resulting binary will fall back to Metal's system default library, which may mismatch the pinned MLX kernels.
...
| `AppState.swift` | `@MainActor` coordinator, per-utterance state machine. Re-reads `Config.customDictionary` per utterance for live updates. Drives overlay clickable re-auth via an injected `oidcInteractiveSignIn` closure (the same `OIDCSessionModel.signIn()` Settings uses) plus `reauthSignIn()` / `reauthReclean()`; tapping cancels the auto-accept timer so the raw transcript can't auto-paste while the browser sign-in is open. |
...
| `OverlayWindow.swift` | Borderless `NSPanel` + SwiftUI. Captures Enter/Esc without stealing focus. In the enterprise-federation fail-closed case (and only that case — keyed off a typed `cleanupFailureReauthable` signal, never the hint copy) the "signed out" failure row is clickable: tapping runs the shared interactive OIDC sign-in (`onReauthSignIn` → AppState), then offers an opt-in ↻ "clean up this dictation" (`onReauthReclean`) that re-runs cleanup on the retained raw transcript. |
...
| `LocalLLMProvider.swift` | On-device cleanup provider (`provider=local`). Acquires the loaded model from `ResidencyManager`, formats the prompt via `LocalTokenizerBridge`, runs MLX generation, and streams token events. Sets `enable_thinking: false` explicitly in the additional-context payload — load-bearing; see invariant #8. |
| `LocalModelStore.swift` | Download + state machine for the on-device model checkpoint (`mlx-community/gemma-4-E4B-it-qat-4bit`). Uses `HubClient` from swift-huggingface; writes a `.parleq-ready` marker only on successful completion; fires `onStateChanged` callbacks so the UI can reflect progress. |
| `ResidencyManager.swift` | Model load/idle-unload lifecycle actor. Holds the loaded `VendoredGemma4Model` (or nil when unloaded); enforces the `ResidencyPolicy` (keep-loaded vs. idle-unload-after-N-minutes based on RAM tier); also carries the `enable_thinking: false` additional-context payload to every generation call. |
| `LocalTokenizerBridge.swift` | Bridges swift-transformers' `AutoTokenizer` to Parleq's prompt format. Applies the Jinja2 chat template from the checkpoint's `tokenizer_config.json`; produces the token-ID array for the generation loop. |
| `VendoredGemma4Text.swift` | **TEMPORARY** in-tree vendor of the Gemma 4 text model graph from mlx-swift-lm, with a KV-shared-layer gating fix applied. Registers `VendoredGemma4Model` under the `"gemma4"` type key so `MLXLLM.ModelRegistry` uses it instead of the upstream (unfixed) version. **Remove this file and its registration in `ResidencyManager` when the upstream mlx-swift-lm fix merges into a tagged release** (`TODO(upstream-gemma4)` comment in the file header). MIT-licensed (Apple). |

## Hard invariants — preserve through refactors

These are non-obvious and worth flagging to anyone editing the codebase:

1. **Audio is memory-only end-to-end.** `AudioRecorder.stop()` returns a `Data`. There is no URL-based path on the dictation hot path. **Do not** add `/tmp/parleq-*.wav` writes or any other audio-on-disk persistence on the dictation path — this is a load-bearing compliance promise. *(**Two documented carve-outs, both Concord/voiceprint or hidden, both off the dictation path:** (1) the opt-in, hidden **contribution-capture mode** writes audio to `~/.parleq/flywheel/` — armed only by hand-editing an exact acknowledgment phrase, off by default, never network-transmitted, absent from all user-facing surfaces (`ContributionRecorder.swift`; §5.3). (2) the **durable-voiceprints enrollment-audio store** (`EnrollmentAudioStore.swift` → `~/.parleq/enrollment-audio.enc`, Concord/release builds only): the user's short voice-**enrollment** carrier clips, kept **AES-GCM-encrypted at rest** (the same Keychain key as `voiceprints.enc`), so a voiceprint can be auto-re-derived across an ASR encoder change without re-enrolling. Local-only, **never network-transmitted**, scoped to enrollment audio (never dictation), gated on a fresh amended consent (`voiceClipStorageConsented`) AND a user/MDM toggle (`voiceprintClipStorageEnabled`) that **fails closed** and erases the store when off. See `docs/SECURITY_REVIEW.md` §5.4. The dictation hot path remains audio-on-disk-free for every user.)*
2. **Transcript content never lands in stderr / log files.** ASR diagnostic is length-only (`(N chars / W words)`). `LocalASR`'s `[vocab]` log is count-only by default; full per-replacement detail is opt-in via `PARLEQ_VOCAB_TRACE=1` env. *(The contribution-capture carve-out in #1 also persists transcript + replacement content — but to the `~/.parleq/flywheel/` corpus, never to logs/stderr; the count-only log contract above is unchanged.)*
...
6. **Audio never leaves the device.** In-process FluidAudio is the only ASR path by default; no local listening socket on the dictation path, no IPC. Cleanup payloads (transcript text) go to the configured LLM provider; that's the only network boundary input data crosses. **When the on-device cleanup option is selected (`provider=local`), cleanup payloads cross no network boundary at all** — transcript text is processed entirely in-process by `LocalLLMProvider` (MLX inference); the only network activity of the local tier is the one-time user-initiated model download from `huggingface.co` (~4 GB, no user content). The optional user-configured `asr.endpoint` (Sherpa-ONNX / faster-whisper running locally) is also a localhost endpoint by convention but the user owns its lifecycle. **One carve-out to "no local listening socket":** the OIDC loopback-redirect sign-in (`LoopbackRedirectServer.swift`) binds a 127.0.0.1-only ephemeral-port listener for the *duration of an active corporate sign-in only* — single callback, static response, defer-based teardown, never logs the callback URL. It does not carry audio. (As of 0.22.0 the sign-in that opens it can be initiated from the overlay's fail-closed "signed out" notice, not just from Settings — so it can exist while a dictation overlay is on screen — but it remains a sign-in-only, single-callback listener that carries no dictation data.) The carve-out is gated **solely** by the redirect_uri (http+loopback selects it; every custom scheme uses `ASWebAuthenticationSession` and binds nothing), so it is fully suppressible by pinning the `oidcRedirectURI` MDM key to a custom-scheme value — there is intentionally no separate "disable loopback" key. Disclosed in `docs/SECURITY_REVIEW.md` §3.4.1.
7. **"Learn from corrections" is opt-in, off by default, and never writes dictation-derived text to disk.** When enabled, `CorrectionJournal` holds correction snippets in an **in-memory ring only** (never written to `~/.parleq/` or anywhere else); `LearningAnalyzer` sends those snippets to the **already-configured** cleanup LLM during periodic off-hot-path analysis (no new network boundary). Analysis log output is count-only — no transcript content in logs. The ring has count + age caps (`learnedCorrectionsMaxEntries` / `learnedCorrectionsRetentionHours`); setting either to `0` disables entirely. Disabling the feature from Settings offers to clear the in-memory ring (and it's cleared on quit regardless). The only on-disk artifact is the learned dictionary terms in `config.json`. *(This is distinct from the **contribution-capture mode** in #1/#2 — a SEPARATE, hidden, acknowledgment-armed component (`ContributionRecorder`) that DOES durably persist raw→final transcript pairs + audio to `~/.parleq/flywheel/`. It is the only path that writes dictation-derived text/audio to disk, and only when explicitly armed.)*
8. **`enable_thinking: false` must be set explicitly on every on-device generation call** (`LocalLLMProvider.swift`, `ResidencyManager.swift`). The Gemma 4 chat template gates an optional hidden chain-of-thought "thinking" channel on a Jinja expression: `enable_thinking is defined and enable_thinking`. If the key is absent from the additional-context dict, the template enables thinking by default, burning the entire token budget on hidden chain-of-thought reasoning before any visible output — catastrophic for latency and output length on the cleanup use case. The explicit `false` is not a nice-to-have: it is load-bearing. Do not remove it, do not omit it when constructing additional-context dicts for new generation call sites.
...
- **Bumping `mlx-swift` requires updating the metallib fetch constants together.** `parleq-app/scripts/fetch-metallib.sh` hardcodes `MLX_SWIFT_VERSION` and `METALLIB_SHA256`. These must be changed in the same commit as the `mlx-swift` pin in `Package.swift`/`Package.resolved`. The script cross-checks the version against `Package.resolved` at build time and will fail loudly if they diverge — but the SHA-256 is not auto-updated; you must fetch the new `Cmlx.xcframework.zip`, compute the SHA-256 of `default.metallib` inside it, and update the constant manually. A mismatched metallib causes subtle kernel failures, not a clean crash.
- **After swapping a SwiftPM path dependency, `rm -rf .build` before building.** Stale object files from a previous path-dep version have caused false "it works" results during investigation — the linker picks up the old binary while the source changes are in place. If you've been working with local path overrides (e.g. testing a local fork of mlx-swift-lm), always do a clean build before drawing conclusions.
