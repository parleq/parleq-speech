...
struct OverlayButtons: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            // The paste-target sits on the LEFT as a descriptive
            // label ("Pasting to:"), and all three actions cluster
            // together on the RIGHT with proper button chrome
            // (.bordered + .borderedProminent) so they read
            // unambiguously as buttons. Earlier iterations used
            // `.plain` controls which didn't read as tappable, and
            // wedged the paste-target between Cancel and Accept
            // which made the row look like a mixed cluster of
            // peer controls.
            HStack(spacing: 8) {
...
#Preview("Key, with paste target") {
    OverlayButtons(isKey: true, pasteTarget: _previewTarget, onCopy: {}, onCancel: {}, onAccept: {})
        .padding()
        .frame(width: 700)
}

#Preview("Key, no paste target") {
    OverlayButtons(isKey: true, pasteTarget: nil, onCopy: {}, onCancel: {}, onAccept: {})
        .padding()
        .frame(width: 700)
}

#Preview("Not key, focus-loss message visible") {
    OverlayButtons(isKey: false, pasteTarget: _previewTarget, onCopy: {}, onCancel: {}, onAccept: {})
        .padding()
        .frame(width: 700)
}
#endif
