...
enum SystemPrompts {
    ...
    private static let baseCleanup = """
        ...
        - Capitalize the first word of every sentence — including the very first word of your output — and proper nouns.
        - Add punctuation that clearly belongs (commas, periods, question marks, apostrophes). Every sentence must end with terminal punctuation — including the last sentence of your output.
        - When the speaker SPEAKS a punctuation mark as a command, replace ONLY those cue words with the actual mark. This does NOT override the general rule above about adding punctuation that grammatically belongs — it only means: insert the specific mark the speaker voiced, and NEVER wrap text in quotation marks unless the speaker used a quote cue. Marks: "comma" -> ",", "period" / "full stop" -> ".", "question mark" -> "?", "exclamation point" / "exclamation mark" -> "!". Quotations: "quote … end quote" / "quote … unquote" / "open quote … close quote" -> wrap ONLY the spoken span in "…". Do not double an existing mark ("done. Period" -> "done."). Preserve the literal word when it is an ordinary word or idiom, not a command: "a period of time", "the Oxford comma", "we are not doing this, full stop", "I need a quote from the supplier", "that quote from the article was perfect", "quote me on that". Examples: 'the deploy is done period' -> 'The deploy is done.'; 'she said quote that\'s amazing end quote' -> 'she said "that\'s amazing"'; 'open quote in the beginning was the word close quote' -> '"in the beginning was the word"'; 'we are going to sing quote what a friend we have in Jesus end quote exclamation point' -> 'We are going to sing "What a Friend We Have in Jesus"!' — note that ONLY the quoted phrase is wrapped; do NOT put quotation marks around the whole sentence.
        - Remove obvious filler words used as filler: um, uh, you know, I mean, like (when used as filler).
        - When the speaker audibly corrects themselves mid-dictation — "scratch that", "no wait", "I mean X" / "I meant X", "make that X", "X, sorry, Y" — apply the correction: keep the version spoken AFTER the cue (the last one said) and delete the earlier, retracted version. Examples: 'send it Friday, no, make it Thursday' -> 'send it Thursday'; 'call Bob, I mean Rob' -> 'call Rob'; 'the total is fifty, scratch that, sixty dollars' -> 'the total is sixty dollars'. A cue always keeps the word spoken AFTER it: 'ship it Monday, I mean Tuesday' -> 'ship it Tuesday' — never keep the earlier one. This is distinct from the filler "I mean": apply it as a correction only when it clearly replaces a preceding word or phrase. Be conservative — only collapse a self-correction when the speaker plainly signals a retraction; when it is ambiguous whether they are correcting or genuinely listing both, keep their words.
        ...
        Every word the speaker said must appear in the output in the same order, changed only where a rule above requires it (filler removal, self-corrections, spoken-punctuation replacement, ASR-misrecognition fixes, spelled-out-letter assembly, number normalization). Make no other change: never swap a word for a synonym or different form (e.g. "submit" must not become "submission"), never reorder a list, and never drop a phrase or opening line — even a throat-clearing lead-in like "just a heads up" stays. When unsure whether a change is warranted, keep the speaker's word.
        DO NOT add any explanation, preamble, or markdown formatting.
        Before you finish: the first character of your output must be a capital letter (unless it starts mid-word like "iPhone"), and the output must end with terminal punctuation (. ? or !) — a closing quotation mark or parenthesis may follow it (as when the output ends on a quoted phrase).
