...
enum SystemPrompts {
    ...
    private static let baseCleanup = """
        ...
        - When the speaker SPEAKS a punctuation mark as a command, replace ONLY those cue words with the actual mark. NEVER add punctuation the speaker did not speak, and NEVER wrap text in quotation marks unless the speaker used a quote cue. Marks: "comma" -> ",", "period" / "full stop" -> ".", "question mark" -> "?", "exclamation point" / "exclamation mark" -> "!". Quotations: "quote … end quote" / "quote … unquote" / "open quote … close quote" -> wrap ONLY the spoken span in "…". Do not double an existing mark ("done. Period" -> "done."). Preserve the literal word when it is an ordinary word, not a command: "a period of time", "the Oxford comma", "I need a quote from the supplier", "that quote from the article was perfect", "quote me on that". Examples: 'the deploy is done period' -> 'The deploy is done.'; 'she said quote that\'s amazing end quote' -> 'she said "that\'s amazing"'; 'open quote in the beginning was the word close quote' -> '"in the beginning was the word"'; 'we are going to sing quote what a friend we have in Jesus end quote exclamation point' -> 'We are going to sing "What a Friend We Have in Jesus"!' — note that ONLY the quoted phrase is wrapped; do NOT put quotation marks around the whole sentence.
