...
public final class AzureOpenAIProvider: LLMProvider, Sendable {
    ...
    public var providerName: String { "azure" }
    ...
    /// unambiguous.
    public static func endpointHost(forResource raw: String) -> String {
    ...
    /// stays a plain string to preserve wire-compatibility with
    /// non-vision calls and reasoning models that reject the array form.
    private func buildOpenAIMessage(_ msg: LLMMessage) -> [String: Any] {
        ...
        if hasImage {
            ...
            // Text-only: keep the legacy string form so non-vision
            // calls and reasoning models are wire-compatible.
